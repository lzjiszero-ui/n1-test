...
export async function GET(request: Request) {
  ...
  const [attempts, profile, sessions] = await Promise.all([
    db()
      .prepare(`SELECT question_id AS id, chosen, seconds, correct, mode,
      session_id AS sessionId, created_at AS createdAt FROM attempts
      WHERE device_id = ? ORDER BY created_at ASC LIMIT 3000`)
      .bind(deviceId)
      .all(),
    db()
      .prepare(`SELECT exam_date AS examDate, daily_minutes AS dailyMinutes,
      target_score AS targetScore FROM study_profiles WHERE device_id = ?`)
      .bind(deviceId)
      .first(),
    db()
      .prepare(`SELECT id, mode, total_questions AS totalQuestions,
      ...
      WHERE device_id = ? ORDER BY completed_at DESC LIMIT 100`)
      .bind(deviceId)
      .all(),
  ]);
...
export async function POST(request: Request) {
  ...
  if (body.attempt) {
    ...
    await db()
      ...
      chosen=excluded.chosen, correct=excluded.correct, seconds=excluded.seconds`)
      .bind(
        deviceId,
        a.sessionId,
        a.id,
  ...
  if (body.profile) {
    ...
    await db()
      .prepare(`INSERT INTO study_profiles
      ...
      .bind(deviceId, p.examDate, p.dailyMinutes, p.targetScore, now)
      .run();
  }
  if (body.session) {
    const s = body.session;
    await db()
      .prepare(`INSERT OR REPLACE INTO learning_sessions
